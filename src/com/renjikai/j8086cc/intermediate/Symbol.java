...
public class Symbol {
	...
	public boolean initialized; // Only char array can be initialized
	...
	public Symbol(String name, int dataType, int scope, boolean isParameter) {
		...
		if (isParameter)
			this.allocateSize = InterDefines.stackDataSize;
	}
	...
	public int getElementAddress(ArrayList<Integer> arrayDimIndex) {
		if (!this.isArray || arrayDimIndex == null)
			return 0;
		int elementIndex = 0;
	...
	public String toString() {
		String varType;
		varType = String.format(InterDefines.varTypeFormat, typeID[this.dataType], this.allocateSize,
				this.funcName + this.name);
		if (this.scope == SymbolTable.TMP_VAR) {
			varType = String.format(InterDefines.varTmpTypeFormat, typeID[this.dataType], this.allocateSize, this.name);
		}
		String formatRef;
		switch (this.scope) {
