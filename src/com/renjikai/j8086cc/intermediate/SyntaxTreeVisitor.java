...
public class SyntaxTreeVisitor extends j8086ccBaseVisitor<String> {
	...
	public String genArrayShiftSize(Symbol s,j8086ccParser.LeftValueContext ctx) {
		...
		result+=String.format(InterDefines.MOVI, 
				sum.toString(), 0
				)+"\n";
		for(int i=0;i<ctx.expr().size();i++) {
			...
			result+=String.format(InterDefines.MOVI, 
					mult.toString(), s.getDimensionSize(i)
					)+"\n";
			result+=String.format(InterDefines.MUL, 
					mult.toString(), mult.toString(), subS.toString()
					)+"\n";
			result+=String.format(InterDefines.ADD, 
					sum.toString(), sum.toString(), mult.toString()
					)+"\n";
		}
		Symbol dataSize=SyntaxTreeHelper.genIntLiteralNode(tmpVarCnter);
		result+=String.format(InterDefines.MOVI, 
				dataSize.toString(), s.dataSize
				)+"\n";
		result+=String.format(InterDefines.MUL, 
				sum.toString(), sum.toString(), dataSize.toString()
				)+"\n";
	...
	public String visitBlock(j8086ccParser.BlockContext ctx) {
		String res="";
		if(ctx.statement()==null)return res;
	...
	public String visitIfSyntax(j8086ccParser.IfSyntaxContext ctx) {
		...
		String elseStmt="";
		if(ctx.statement().size()!=1)elseStmt=visit(ctx.statement(1));
	...
	public String visitLValueAssign(j8086ccParser.LValueAssignContext ctx) {
		...
		if(!s.isArray) {
			String expr=visit(ctx.expr());
			Symbol exprResult=exprSymbol.get(ctx.expr());
			String movOp=String.format(InterDefines.MOV, s.toString(),
					exprResult.toString())+"\n";
			return expr+movOp;
		}else { // isArray
			...
			String addShiftIns=String.format(InterDefines.ADD,
					arrBaseAddr.toString(),arrBaseAddr.toString(),
					arrShiftAddr.toString()
					)+"\n";
			String expr=visit(ctx.expr());
			Symbol exprResult=exprSymbol.get(ctx.expr());
			String movMROp=String.format(InterDefines.MOVMR,
					arrBaseAddr.toString(),exprResult.toString())+"\n";
	...
	public String visitLogicOrExpr(j8086ccParser.LogicOrExprContext ctx) {
		...
		String com=String.format(InterDefines.LOR, 
				sc.toString(),s1.toString(),s2.toString()
				)+"\n";
	...
	public String visitEqExpr(j8086ccParser.EqExprContext ctx) {
		...
		if(ctx.op.getType()==j8086ccParser.EQ) {
			com=String.format(InterDefines.EQ, 
					sc.toString(),s1.toString(),s2.toString()
					)+"\n";
		}else { //NE
			com=String.format(InterDefines.NE, 
					sc.toString(),s1.toString(),s2.toString()
					)+"\n";
		}
		return str1+str2+com;
	...
	public String visitUnaryExpr(j8086ccParser.UnaryExprContext ctx) {
		...
		if(ctx.op.getType()==j8086ccParser.ADD) {
			com=String.format(InterDefines.MOV, 
					sc.toString(),s.toString()
					)+"\n";
		}else if(ctx.op.getType()==j8086ccParser.SUB) {
			com=String.format(InterDefines.MOVI, 
					sc.toString(),0
					)+"\n";
			com+=String.format(InterDefines.SUB, 
					sc.toString(),sc.toString(),s.toString()
					)+"\n";
		}else { //NOT
			com=String.format(InterDefines.LNOT, 
					sc.toString(),s.toString()
					)+"\n";
		}
		return str+com;
	...
	public String visitLogicAndExpr(j8086ccParser.LogicAndExprContext ctx) {
		...
		Symbol sc=SyntaxTreeHelper.genTmpFatherNode(tmpVarCnter, s1, s2);
		exprSymbol.put(ctx, sc);
		String com=String.format(InterDefines.LAND, 
				sc.toString(),s1.toString(),s2.toString()
				)+"\n";
		return str1+str2+com;
	}
	...
	public String visitIntLiteral(j8086ccParser.IntLiteralContext ctx) {
		...
		return String.format(InterDefines.MOVI, s.toString(),
	...
	public String visitLValueExpr(j8086ccParser.LValueExprContext ctx) {
		...
		if(!s.isArray) {
			...
			String movRMOp=String.format(InterDefines.MOVRM,
					curS.toString(),arrBaseAddr.toString())+"\n";
	...
	public String visitRelExpr(j8086ccParser.RelExprContext ctx) {
		...
		if(ctx.op.getType()==j8086ccParser.LT) {
			com=String.format(InterDefines.LT, 
					sc.toString(),s1.toString(),s2.toString()
					)+"\n";
		}else if(ctx.op.getType()==j8086ccParser.LE) {
				com=String.format(InterDefines.LE, 
						sc.toString(),s1.toString(),s2.toString()
						)+"\n";
		}else if(ctx.op.getType()==j8086ccParser.GE) {
			com=String.format(InterDefines.GE, 
					sc.toString(),s1.toString(),s2.toString()
					)+"\n";
		}else { //GT
			com=String.format(InterDefines.GT, 
					sc.toString(),s1.toString(),s2.toString()
					)+"\n";
		}
		return str1+str2+com;
	...
	public String visitHighArithExpr(j8086ccParser.HighArithExprContext ctx) {
		...
		if(ctx.op.getType()==j8086ccParser.MUL) {
			com=String.format(InterDefines.MUL, 
					sc.toString(),s1.toString(),s2.toString()
					)+"\n";
		}else if(ctx.op.getType()==j8086ccParser.DIV) {
				com=String.format(InterDefines.DIV, 
						sc.toString(),s1.toString(),s2.toString()
						)+"\n";
		}else { //MOD
			com=String.format(InterDefines.MOD, 
					sc.toString(),s1.toString(),s2.toString()
					)+"\n";
		}
		return str1+str2+com;
	...
	public String visitLowArithExpr(j8086ccParser.LowArithExprContext ctx) {
		...
		String com;
		if(ctx.op.getType()==j8086ccParser.ADD) {
			com=String.format(InterDefines.ADD, 
					sc.toString(),s1.toString(),s2.toString()
					)+"\n";
		}else { //SUB
			com=String.format(InterDefines.SUB, 
