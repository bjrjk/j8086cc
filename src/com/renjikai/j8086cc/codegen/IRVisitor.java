...
public class IRVisitor extends j8086cInterBaseVisitor<String> {
	...
	public Integer getVarBPShiftPostion(String funcName, String varName, j8086cInterParser.VarNameContext ctx) {
		if (varBPShift.containsKey(varName))
			return varBPShift.get(varName);
	...
	public String getFuncName(ParserRuleContext ctx) {
		while (ctx != null) {
			if (ctx instanceof j8086cInterParser.FunctionContext) {
				j8086cInterParser.FunctionContext ctxF = (j8086cInterParser.FunctionContext) ctx;
	...
	public String visitDataSegment(j8086cInterParser.DataSegmentContext ctx) {
		...
		for (int i = 0; i < ctx.varDecl().size(); i++) {
			res += visit(ctx.varDecl(i));
		}
		return res;
	}

	@Override
	public String visitVarDecl(j8086cInterParser.VarDeclContext ctx) {
		if(ctx.STRING() == null)
			return String.format("%s DB %d Dup(0)\n", 
					ctx.varName().IDENTIFIER().getText(), 
					Integer.valueOf(ctx.varName().INT().getText()));
		else {
			int arrayLen = Integer.valueOf(ctx.varName().INT().getText());
			String strContent = ctx.STRING().getText();
			strContent = strContent.substring(1, strContent.length() - 1);
			int initializedArrLen = strContent.length();
			int leftArrLen = arrayLen - initializedArrLen;
			return String.format("%s DB '%s', %d Dup(0)\n", 
					ctx.varName().IDENTIFIER().getText(), 
					strContent,
					leftArrLen);
		}
	...
	public String visitFunction(j8086cInterParser.FunctionContext ctx) {
		...
		for (int i = 0; i < ctx.statement().size(); i++) {
			if (!(ctx.statement(i) instanceof j8086cInterParser.LocalVarDeclContext))
				continue;
			j8086cInterParser.VarNameContext curVar = ((j8086cInterParser.LocalVarDeclContext) ctx.statement(i))
					.varDecl().varName();
		...
		String funcEnd = (funcName.equals("main") ? "mov AH,4Ch\nmov AL,0\nint 21h\n"
				: String.format("RESTORE_SITE %d\n", paramCount * 2)) + String.format("%s ENDP\n", funcName);
