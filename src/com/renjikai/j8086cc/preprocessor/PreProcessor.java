		...
		for (String codeLine : lines) {
			if (codeLine.startsWith("#include")) {
				...
				if (!j8086cHeaderFileName.endsWith(".j8086h")) {
					Logger.throwError("PreProcessor: Macro INCLUDE header file not end with .j8086h! \n"
				...
				try {
					...
					Logger.throwError("PreProcessor: Macro INCLUDE header file IO error! \n" + "For the file: "
							+ j8086cHeaderFileName);
				}
			} else {
				buf.append(codeLine + "\n");
			}
